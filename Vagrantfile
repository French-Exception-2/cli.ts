...
# Load Configuration
  ...
  vagrant_config_local = JSON.parse(File.read(File.join(dir,"\\config-local.json")))
...
# Global
  ...
  vagrant_config["nodes"].each do |servername, server|
    ...
    (0..instances_n-1).each { |instance| 
      ...
      ip = server['ip_pattern'].gsub("\#{INSTANCE}", (instance+server["ip_start"]).to_s)
     
      servername_real = server["hostname_pattern"]
        .gsub("\#{INSTANCE}", instance_str)
        .gsub("\#{NAME}", servername)
        .gsub("\#{VAGRANT_INSTANCE}", format('%02d', vagrant_config['vagrant']['instance']))
      ...
      config.vm.define servername_real, primary: (server['primary'] && instance == 0) do |srv|
        ...
        srv.vm.provider "virtualbox" do |vb|
          ...
          vb.customize ["modifyvm", :id, "--accelerate3d", (server['3d'] ? "on" : "off")]
        ...
        if (server['provisioning'])
          ...
          server['provisioning'].each do |key,value|
            ...
            srv.vm.provision "shell", name: key, 
                                      path: "./.vagrant/provisioning/#{os_type}/#{os_version}/#{key}.#{extension}", 
                                      env: env, 
